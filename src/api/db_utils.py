"""
Database utilities for rag-graph-2024, conversation memory and document
records on Postgres via psycopg 3.

This module keeps the public interface of the SQLite memory layer used by the
earlier rungs of the line (rag-naive-2022, rag-advanced-2023) while swapping
the storage engine to Postgres, the same instance used for pgvector.
...
def init_db() -> None:
    ...
    from src.kag.graph_store import create_kg_edges
