...
graph facts it used, so callers and the UI can see the structured reasoning.
"""
...
from src.kag.graph_store import delete_edges
...
def chat(request: Request, query: QueryInput, _: str = Depends(verify_api_key)):
    ...
    result = run_kag(query.model, question, history)
