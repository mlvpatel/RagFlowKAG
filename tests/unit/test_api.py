...
def test_chat_runs_kag_and_persists_the_turn(client, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda model, q, history: {
            "answer": "Hello world",
            "steps": [{"step": "vector_retrieval", "chunks": 3}],
            "entities": ["Acme"],
            "facts": ["Acme offers 15 vacation days"],
        },
    )
    persisted = {}
    monkeypatch.setattr(
    ...
    assert body["steps"][0]["step"] == "vector_retrieval"
    assert body["entities"] == ["Acme"]
    assert body["facts"] == ["Acme offers 15 vacation days"]
