...
def test_settings_has_documented_defaults():
    ...
    assert settings.database_url == "postgresql://rag:rag@localhost:5432/rag_graph"
...
def test_env_var_override_case_insensitive(monkeypatch):
    monkeypatch.setenv("kag_neighbor_limit", "40")

    settings = Settings()

    assert settings.kag_neighbor_limit == 40
...
def test_yaml_file_overrides_defaults(tmp_path, monkeypatch):
    ...
    assert settings.kag_neighbor_limit == 20
